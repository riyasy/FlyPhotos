...
/// <summary>
/// Low-level P/Invoke declarations for the Rust fly_rust_bridge.dll.
/// All interop entry-points are collected here; higher-level logic lives in <see cref="RawlerWrapper"/>.
/// </summary>
internal static partial class NativeRustBridge
{
    ...
    internal static partial nint rawler_render_raw(string path, out int width, out int height, out int rotation);

    [LibraryImport(DllName, StringMarshalling = StringMarshalling.Utf8)]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    internal static partial nint rawler_get_embedded_preview_from_raw(string path, out int width, out int height, out int rotation, out int primaryWidth, out int primaryHeight);

    [LibraryImport(DllName, StringMarshalling = StringMarshalling.Utf8)]
    internal static partial nint resvg_render_svg(string path, int maxDimension, out int width, out int height);
